...
public:  
  ...
  */
   ...
   */
  /**
    auto_pk_next_insert_id is the next value which should be inserted into the
    auto_pk column: in a inserting-multi-row statement (like INSERT
    SELECT), for the first row where the auto_pk_inc value is not specified by the
    statement, get_auto_pk_increment() called and asked to generate a value,
    auto_pk_next_insert_id is set to the next value, then for all other rows
    auto_pk_next_insert_id is used (and increased each time) without calling
    get_auto_pk_increment().
  */
  ulonglong auto_pk_next_insert_id;
  /**
    insert id for the current row (*autogenerated*; if not
    autogenerated, it's 0).
    At first successful insertion, this variable is stored into
    THD::first_successful_insert_id_in_cur_stmt.
  */
  ulonglong auto_pk_insert_id_for_cur_row;
  /**
    Interval returned by get_auto_pk_increment() and being consumed by the
    inserter.
  */
  Discrete_interval auto_pk_inc_interval_for_cur_row;
  /**
     Number of reserved auto primary key increment intervals. Serves as a heuristic
     when we have no estimation of how many records the statement will insert:
     the more intervals we have reserved, the bigger the next one. Reset in
     handler::ha_release_auto_pk_increment().
  */
  uint auto_pk_inc_intervals_count;
...
public:
  ...
  */
   ...
   * auto_pk
   */
  int update_auto_pk_increment();
  virtual void get_auto_pk_increment(ulonglong offset, ulonglong increment,
                                  ulonglong nb_desired_values,
                                  ulonglong *first_value,
                                  ulonglong *nb_reserved_values);
  void set_auto_pk_next_insert_id(ulonglong id)
  {
    DBUG_PRINT("info",("auto_pk_increment: next value %lu", (ulong)id));
    auto_pk_next_insert_id= id;
  }
